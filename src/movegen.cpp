	...
	if (isPawn%6 != 0){
		while (Target != 0){
			...
			if ((BIT1 >> to & Enemy) != 0 )	{ 
				...
				FLG = QUIET_MOVES;	
				moveList->value	= EVALUATION::PieceSquareValue(isPawn, from, to);	
			}
			flags = FLG;
		...
		while (Target != 0){
			...
			if ( ( ( PawnDes & Rank1 ) || ( PawnDes & Rank8 ) ) != 0 ){
				...
				if ((BIT1 >> to & Enemy) != 0 )	{	
					FLG = CAPTURE;		//actually capture by pawn is super valuable almost all the time;
					int a =	EVALUATION::SEEA(to, C, from);
					...
					if (DoublePush == 16 || DoublePush	==	-16)	
						FLG	= DOUBLE_PUSH;
					else  											
						FLG	= QUIET_MOVES;	
					int a = 0; //EVALUATION::SEEA(to, C, from);
					moveList->value	= EVALUATION::PieceSquareValue(isPawn, from, to) + a;
				}
				flags = FLG;
				moveList->move = (from | to << 6 | flags << 12);
...
/*
	add moves and perform shallow evaluation
*/
	...
	if (isPawn%6 != 0){ //if piece is not pawn
		while (target != 0){
			...
			if ((BIT1 >> to & enemy) != 0 )	{	
				...
				FLG	= QUIET_MOVES;	
				newMove.value = EVALUATION::PieceSquareValue(isPawn, from, to);	
			}
			flags = FLG;
